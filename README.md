...
| `RESEND_API_KEY` | **The transport itself, and the variable that gates all auth email.** Verification and password-reset mail go through a [Resend](https://resend.com) proxy the runtime keeps to itself (the key never reaches an app isolate). Unset — which is the default — the proxy answers `503` and neither flow can send. Optional: signup and login work fine without it, because email verification is off unless you turn it on. |
| `EXEPAD_EMAIL_FROM` / `EXEPAD_EMAIL_FROM_NAME` | From-address (and display name) for that mail. **Also required to actually deliver:** the built-in default is an `@exepad.com` address you cannot verify with your own Resend account, so a send with it is rejected even when the key is valid. Use a domain you control and have verified. |
