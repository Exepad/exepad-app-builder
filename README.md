...
The first screen asks for a **setup token** — so that if your machine is
reachable on a network, nobody else can claim your studio before you do. Exepad
prints it when it starts; read it back any time with:

```bash
docker logs exepad
```

It is in the startup banner, near the top. (Seeding `EXEPAD_ADMIN_EMAIL` and
`EXEPAD_ADMIN_PASSWORD` creates the account for you and skips this entirely.)
