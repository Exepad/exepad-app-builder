...
#   2. Node >= 18 present -> delegate to the canonical `npx exepad up`.
...
function Invoke-NpxDelegation {
  ...
  $pkg = 'exepad'
  if (Test-Semver $Version) { $pkg = "exepad@$Version" }
...
"@
  ...
  if ($ReleaseChannel -eq 'public') {
    Say '  operator CLI: npx exepad status | update | stop | start | backup   (needs Node)'
  }
}
