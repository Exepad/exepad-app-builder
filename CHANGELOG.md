...
`exepad` npm launcher, and the per-platform installers on the GitHub Release all
begin existing here.
...
- **The install docs lead with the one-liners, because as of this release they
  ...
  reference had documented `curl | bash`, `irm | iex`, `npx exepad up` and the
...
- **`docker compose` put the data volume somewhere backups didn't look.** The
  ...
  **Upgrade note (private 0.x only):** if you ran `docker compose up` from a
  ...
  Installs made by `install.sh`, `install.ps1` or `npx exepad up` were always on
...
[Unreleased]: https://github.com/Exepad/exepad-app-builder/compare/v1.0.0...HEAD
