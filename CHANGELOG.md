...
**Setup stops asking you for things.** The installers now create your operator
account and hand the studio a working provider key, so first run opens on a
usable studio instead of a token prompt and an empty preview. A minor rather than
a patch release: the studio gains the provider-setup form and the prompt-box
guard, and the installers change what they do rather than only how they report.

### Added

- **The studio now asks for your AI provider setup instead of showing an empty
  preview.** Without a key the agent cannot build anything, so the preview pane's
  "Your preview will appear here." was promising something that could never
  ...
  The pane now offers the **same form as Settings' "AI engine (LLM)" card** —
  every provider (OpenRouter, Gemini, Anthropic, OpenAI, and custom
  OpenAI-compatible endpoints), the key, an optional model with the OpenRouter
  catalogue, and the base URL where one is needed. The provider catalogue is
  shared between the two screens so they cannot drift. A key supplied via
  `EXEPAD_LLM_API_KEY` already reports as set (`source: 'env'`), so an install
  that passed `--llm-key` never sees it.

- **The prompt box is disabled until a provider key exists**, rather than
  accepting a build that was always going to fail. The guard is in the submit
  path, not just on the controls: Enter is handled on keydown, and a disabled
  button is a UI state rather than a lock.

### Fixed

- **`docker build` on Windows produced an image that could not start.** The
  documented build-from-source path died instantly with
  `env: 'bash\r': No such file or directory` — Git for Windows defaults to
  `core.autocrlf=true`, `.gitattributes` had no rule for `*.sh`, and so
  `docker/entrypoint.sh` was baked into the image with CRLF line endings. Linux
  and macOS clones normalise on checkout and CI builds on Linux, so nothing ever
  reproduced it; it was reachable only by the Windows users the build
  instructions invite. Shell scripts, `*.command` and the Caddyfiles are now
  pinned `text eol=lf` (the Windows one-click wrappers keep their deliberate
  `-text` CRLF, and are listed after so they still win). Found by running the
  repo's own build instructions on a Windows box.

### Changed

- **The installers now create your operator account, so the studio opens at an
  ordinary login.** They ask for the email and password you want, seed
  `EXEPAD_ADMIN_EMAIL`/`EXEPAD_ADMIN_PASSWORD`, and the container creates the
  account at boot — which *closes* first-run setup. That beats every way of
  guarding an unclaimed instance, because there is no unclaimed instance: the
  window in which the studio offers to make an admin account to whoever reaches
  it never opens. Enter at the email prompt skips and falls back to creating it
  in the browser.

  This is not the AI-key prompt returning. That was configuration — changeable
  later, and Settings is its home. This is ownership, which has to be settled
  before someone else settles it. The masked-input trap is the same either way,
  so the prompt says "your typing will NOT be shown" *before* it masks.

  Non-interactive paths never prompt: `--yes`, `--admin-email`/
  `--admin-password` already given, no TTY (piped, CI, a service), or a re-run
  over an existing install. Password validation is enforced installer-side
  because `seedAdminFromEnv` calls `createUser` directly and — unlike
  `POST /auth/setup`, which requires 8 characters — enforces no minimum at all;
  without the check the seeded path would have been quietly weaker than the form
  it replaces.

  Once the account is confirmed to exist, **the password is removed from
  `.env`**. It is hashed into the data volume by then and `seedAdminFromEnv`
  no-ops from that point, so the plaintext earns nothing by staying. Stripping
  happens only *after* confirmation — removing a password whose seed silently
  failed would strand the user with no account and no way to seed one.

- **First-run setup no longer asks for a setup token.** The installers now set
  `EXEPAD_ALLOW_OPEN_SETUP=1`, and the studio hides the field, so creating your
  operator account is an email and a password. The token proved "I can read
  this host's logs" — a stand-in for "I am the operator" — which for the
  ordinary case meant making someone dig through `docker logs` to prove they
  own the laptop they are sitting at. Detecting "the request came from this
  machine" instead is not available to us: with a published port, Docker SNATs
  every connection to the bridge gateway, so the container sees the host's own
  browser and a stranger on the LAN as the same `172.x.0.1`.

  **What this trades away:** until you complete the setup form, anyone who can
  reach the published port can complete it instead of you — the container
  publishes on all interfaces. Create your account right after installing. The
  installers now say so on their last line.

  Reversible, and never overwritten: put `EXEPAD_ALLOW_OPEN_SETUP=0` in
  `~/.exepad/.env` and re-run — no later run clobbers an explicit setting. The
  token comes back, and the installer prints it. For anything internet-facing,
  prefer seeding `EXEPAD_ADMIN_EMAIL` + `EXEPAD_ADMIN_PASSWORD`, which closes
  setup outright.

- **No installer asks you to configure anything any more.** `install.ps1`,
  `install.sh` and `exepad up` each stopped mid-install to ask for the AI
  provider key. Because it is a secret the prompt had to hide the typing, which
  on a fresh machine is indistinguishable from a hang — and the two paths that
  never pass `--yes` are exactly the beginner ones: the one-click
  `Install Exepad.bat` and the MSI, both of which just sat at a blinking cursor.
  Configuration now lives only in the studio's Settings UI, which is also the
  only place it can be validated and rotated. `--llm-key` / `-LlmKey` /
  `EXEPAD_LLM_API_KEY` still work for unattended installs, which have no UI to
  go to. Having no key yet is now reported once at the end, as a next step,
  rather than as a warning mid-install.

### Fixed

- **Windows: the first-run setup token was never printed.** The install would
  succeed, the studio would come up demanding a setup token, and the installer
  had already silently thrown away the only copy the user was going to be
  shown — leaving `docker logs exepad`, which no one has a reason to know, as
  the only way in. On Windows PowerShell 5.1 (the OS default, and what both the
  `.bat` and the MSI invoke), *redirecting* a native command's stderr wraps each
  line in a `NativeCommandError` record, and `$ErrorActionPreference = 'Stop'`
  promotes the first one to a terminating error. So `docker logs … 2>&1` did not
  return the logs; it threw, and the surrounding `try/catch` swallowed it. Every
  native capture now goes through one guarded helper. If the token still cannot
  be found, the installer names the command that prints it instead of saying
  nothing.

- **Windows: a re-run with an API key silently discarded it.** In the `.env`
  merge, a pipeline yielding exactly one surviving line returns a `[string]`,
  not an array — so `+=` concatenated instead of appending, and the file came
  out as `# my notesEXEPAD_LLM_API_KEY=sk-…`, with the key folded into a comment
  and lost. Triggered by the ordinary case of re-running the installer over an
  existing `.env`.

- **Windows: Docker Desktop installed but not running is no longer a failed
  install.** That is the normal state on any machine that has met Docker before,
  since it does not start with the session unless you opt in. The installer now
  starts Docker Desktop itself and waits for the engine (checking both the
  per-machine and the newer per-user install locations), instead of telling the
  user to go and do it and then aborting.

- **Windows: re-installing the same version warned that a pinned tag was "a
  moving tag".** The downgrade guard's conditions were arranged so that an exact
  version match fell through to the moving-tag branch. Re-running the same
  version is a repair or a resume — it is what the MSI's *Install or Update
  Exepad Studio* shortcut does every time — and is now silent.

- **Windows: the install died mid-pull when its output was redirected.**
  `docker compose pull` writes progress to stderr, so running the installer into
  a log file or from another tool aborted it with `error:  Image … Pulling`,
  which names no problem at all.

- **Windows: the npm-launcher handoff never fired.** The `node` and `npx` probes
  used the same unguarded stderr redirect, so both were read as "no usable
  launcher" and every Node machine silently fell back to the embedded bootstrap.

### Added

- `packaging/one-click/windows/install-ps1.tests.ps1` — behavioural tests for
  `install.ps1` that run under Windows PowerShell 5.1 in `packaging-ci`. The
  existing checks parsed the script and ran it with `-DryRun`, which writes no
  `.env` and returns early from the setup-token path, so it exercised neither of
  the two defects above. These load the real functions out of the shipped file
  and drive them directly — no Docker, no network — and guard against the
  configuration prompt returning.
...
[Unreleased]: https://github.com/Exepad/exepad-app-builder/compare/v1.1.0...HEAD
[1.1.0]: https://github.com/Exepad/exepad-app-builder/releases/tag/v1.1.0
