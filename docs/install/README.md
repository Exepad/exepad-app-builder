...
> the launcher is the `exepad` npm package, and the image is
...
Once the launcher is published, `npx exepad doctor` checks a host for you;
...
The path that works today, and the one the README and
[INSTALL.md](../../INSTALL.md) lead with: build the same image a release ships
and run it exactly like the released one. Needs only Docker and git — no
registry, no npm package, no accounts (set the LLM key afterwards in the in-app
**Settings**):
...
> **This section is gated on the first public release.** The `exepad` npm
> package, the `ghcr.io/exepad/exepad-app-builder` image, and the GitHub Release
> assets are all published by the first `vX.Y.Z` tag — until that tag is cut
> there is nothing for the commands below to fetch (the installer host does not
> resolve, and the npm/GHCR names have nothing behind them), so they fail. Use
> [Quick start — build from source](#quick-start--build-from-source) above; it
> produces the same image. This is the canonical explanation — the README,
> [INSTALL.md](../../INSTALL.md), and the
> [deployment reference](../latest/10-deployment.md) link here rather than
> repeating it.
...
delegates to `npx exepad`, otherwise it runs a self-contained bootstrap that
...
```bash
npx exepad update --to X.Y.Z   # backs up the data volume, then pulls + restarts (guards downgrades)
npx exepad backup              # archive the data volume to a .tgz
npx exepad stop | start | restart | down | logs -f | doctor | status
```

For a persistent command: `npm i -g exepad`, then `exepad up`.
...
> `EXEPAD_HOME=~/exepad npx exepad up` (see [Troubleshooting](#snap-docker)).
...
```bash
npx exepad up --domain app.internal.company --tls byoc
...
- **Lockstep.** `npx exepad@X.Y.Z up` installs studio **X.Y.Z** (launcher version ==
...
```bash
npx exepad update --to X.Y.Z     # backs up, then upgrades
npx exepad restore <backup.tgz> --to <older-version>   # the rollback path
```
...
```bash
npx exepad backup        # → ~/.exepad/backups/exepad-data-<stamp>.tgz
...
Run **`npx exepad doctor`** first — it checks Docker, the daemon, architecture,
...
install dir fails. Use a non-hidden dir: `EXEPAD_HOME=~/exepad npx exepad up`.
...
```bash
npx exepad down                       # stop + remove the container (keeps data)
