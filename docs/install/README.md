...
2. Create the operator account (or pre-seed it with `EXEPAD_ADMIN_EMAIL` /
   `EXEPAD_ADMIN_PASSWORD`). Do it promptly — until that form is filled in,
   anyone who can reach the port can fill it in instead of you.

   The installers set `EXEPAD_ALLOW_OPEN_SETUP=1`, so this is just an email and
   a password. A container you started yourself (the `docker run` above) has no
   such setting, so it demands the **setup token** printed in its startup
   banner — read it back with `docker logs exepad` (works in bash and
   PowerShell alike; `grep` does not exist on Windows). Add
   `-e EXEPAD_ALLOW_OPEN_SETUP=1` to skip it, or set it to `0` in
   `~/.exepad/.env` to require it on an installer-managed instance.
...
# && chained: a failed stop must not fall through to tarring a live database.
docker stop exepad && \
  docker run --rm -v exepad-data:/data:ro -v "$PWD":/backup alpine \
    tar czf /backup/exepad-backup.tgz -C /data . && \
  docker start exepad

# Restore. The container MUST be stopped for this too — untarring over
# databases a running instance holds open corrupts them, and the restored
# files would be overwritten by whatever that process flushes next.
# This replaces the CURRENT volume; `exepad restore` does the same with a
# confirmation prompt.
docker stop exepad
