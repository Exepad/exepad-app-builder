...
1. **What** — choose *a domain I own* or *a public IP*, and what it serves:
   ...
   - **Per-app subdomain (wildcard)** — `*.apps.example.com`; each `{app}.apps…`
     serves that app. This works on the **default** TLS mode: point one wildcard
     A record at the box, and each published app's subdomain gets its own
     trusted certificate on first request — any DNS provider, no API token.
     DNS-01 (§5) is *optional* here.
...
| A subdomain per app | Wildcard (auto — one cert per subdomain, on demand) | `*.apps.acme.com` | `https://invoices.apps.acme.com/` |
...
|---|---|---|
| **auto** (HTTP-01/TLS-ALPN-01) | A public domain — **including a `*.` wildcard**, which gets a separate cert per app subdomain, issued on demand | Inbound `:80` + `:443` reachable. |
| **dns** (DNS-01) | Behind NAT — or when you want ONE pre-issued wildcard cert instead of per-subdomain ones (optional: wildcards also work on **auto**) | A DNS-provider API token on the Caddy sidecar; no open ports. |
