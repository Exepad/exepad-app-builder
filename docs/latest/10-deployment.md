...
Once a `vX.Y.Z` tag has been cut, the release publishes the image to GHCR
(`ghcr.io/exepad/exepad-app-builder`) and the `exepad` launcher to npm in
lockstep; **one command** then installs and starts the studio. npm ships only a
tiny launcher; Docker carries the image. Nothing below exists before that tag —
[what's gated](../install/README.md#once-the-first-release-is-published). See
...
npx exepad up                  # latest;  npx exepad@X.Y.Z up  pins a version
...
```bash
npx exepad update --to X.Y.Z   # backs up the data volume, then pulls + restarts; refuses a downgrade (--force to override)
...
`npx exepad backup` is a one-command wrapper over exactly this. Always back up
before a version change — it's also the escape hatch the downgrade guard points to.
