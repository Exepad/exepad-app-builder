...
// ── "Require email verification" is gated on a real transport ────
// Arming this toggle without an outbound transport does not degrade signup — it
// BREAKS it. app-backend's authSignup withholds the session, tries to mail a
// verification link, and throws InternalError('Could not send verification
// email') when the send fails, so every new user is turned away by an error they
// cannot act on. The route must refuse to arm it. The panel disables the switch
// too, but this is the real gate: the panel is not the only caller.
describe('PUT /:appId/settings — email-verification transport gate', () => {
  /** resolveSecret that reflects the actual binding instead of a blanket stub. */
  function passthroughSecrets() {
    mockResolveSecret.mockImplementation(async (v: unknown) =>
      typeof v === 'string' ? v : '',
    );
  }

  it('refuses to enable it when no transport is configured', async () => {
    passthroughSecrets();
    const { env, putCalls } = makeEnv({
      [`${APP_ID}/preview/app-config.json`]: JSON.stringify({ security: {} }),
    });
    // No RESEND_API_KEY on env — a stock instance.

    const res = await app.request(`/${APP_ID}/settings`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ security: { enabled: true, requireVerification: true } }),
    }, env);

    const { body, status } = await parse(res);
    expect(status).toBe(400);
    expect(body.success).toBe(false);
    // The message has to name the variable, or the operator is stuck.
    expect(body.error).toMatch(/RESEND_API_KEY/);
    // Nothing may be staged, and no redeploy may run.
    expect(putCalls).toHaveLength(0);
    expect(mockDeployFetch).not.toHaveBeenCalled();
  });

  it('allows enabling it once a transport IS configured', async () => {
    passthroughSecrets();
    const { env, putCalls } = makeEnv({
      [`${APP_ID}/preview/app-config.json`]: JSON.stringify({ security: {} }),
    });
    (env as Record<string, unknown>).RESEND_API_KEY = 're_live_stub';
    ...
    expect(res.status).toBe(200);
    const staged = JSON.parse(putCalls[0].value) as { security: Record<string, unknown> };
    expect(staged.security.requireVerification).toBe(true);
  });

  it('always allows turning it OFF, so a stuck instance can recover', async () => {
    passthroughSecrets();
    const { env, putCalls } = makeEnv({
      // Already armed (e.g. set before the gate existed, or by the agent).
      [`${APP_ID}/preview/app-config.json`]: JSON.stringify({
        security: { enabled: true, requireVerification: true },
      }),
    });
    // Still no transport — the gate must not trap the operator in this state.

    const res = await app.request(`/${APP_ID}/settings`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ security: { enabled: true, requireVerification: false } }),
    }, env);

    expect(res.status).toBe(200);
    const staged = JSON.parse(putCalls[0].value) as { security: Record<string, unknown> };
    expect(staged.security.requireVerification).toBe(false);
  });

  it('reports emailConfigured on GET so the panel can explain itself', async () => {
    passthroughSecrets();
    const { env } = makeEnv({
      [`${APP_ID}/preview/app-config.json`]: JSON.stringify({ security: {} }),
    });

    const off = await parse(await app.request(`/${APP_ID}/settings`, undefined, env));
    expect(off.body.data.emailConfigured).toBe(false);

    (env as Record<string, unknown>).RESEND_API_KEY = 're_live_stub';
    const on = await parse(await app.request(`/${APP_ID}/settings`, undefined, env));
    expect(on.body.data.emailConfigured).toBe(true);
  });
});
