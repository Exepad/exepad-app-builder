...
/**
 * Is there a working outbound transport for per-app auth email?
 *
 * "Require Email Verification" withholds the session at signup and mails a
 * verification link; if that send throws, `authSignup` fails the whole signup
 * with `Could not send verification email` (app-backend
 * `auth/handlers/signup.ts`). So enabling the toggle on an instance with no
 * transport does not degrade signup — it BREAKS it, for every new user, with an
 * error that names nothing the operator can act on.
 *
 * The transport is the Resend proxy at `/api/platform/email/send`, which answers
 * 503 when `RESEND_API_KEY` is unset. Nothing sets it by default, so this is the
 * state of a stock instance.
 */
async function isEmailTransportConfigured(env: Env): Promise<boolean> {
  try {
  ...
  return c.json({
    ...
    data: {
      ...
      models: Array.isArray((backend.models as unknown[]))
        ...
        : [],
      // Lets the panel disable "Require Email Verification" and say WHY, instead
      // of offering a switch whose only effect would be to break signup.
      emailConfigured: await isEmailTransportConfigured(c.env),
    },
  });
});
...
settings.put('/', async (c) => {
  ...
  // Refuse to arm a toggle that would break signup. The UI disables it too, but
  // this is the real gate — the panel is not the only caller of this route, and
  // the failure it prevents lands on the app's END USERS, who cannot see or fix
  // the cause. Only blocks turning it ON; clearing it is always allowed, so an
  // instance that somehow has it set can always recover.
  if ((body.security as Record<string, unknown>).requireVerification === true) {
    if (!(await isEmailTransportConfigured(c.env))) {
      return c.json(
        {
          success: false,
          error:
            'Email verification needs an outbound email transport, and none is configured. ' +
            'Set RESEND_API_KEY (plus EXEPAD_EMAIL_FROM on a domain you have verified with ' +
            'Resend) and restart, then enable this. Without it every new signup would fail ' +
            'with "Could not send verification email".',
        },
        400,
      );
    }
