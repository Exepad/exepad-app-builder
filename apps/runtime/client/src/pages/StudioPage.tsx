...
export default function StudioPage() {
  ...
  // Is an AI provider key configured? `null` while we are still asking, so the
  // pane shows neither the gate nor the empty state during that flicker.
  // Without a key the agent cannot build at all, so this gates the studio.
  const [llmKeySet, setLlmKeySet] = useState<boolean | null>(null);
  ...
  // Ask once on mount whether a provider key exists. A failed/unauthorised
  // request resolves to `true`: the studio must never be gated by a check that
  // could not be made, and a real missing key surfaces anyway when a build runs.
  const refreshLlmKeyState = useCallback(async () => {
    const settings = await getSettings();
    setLlmKeySet(settings ? Boolean(settings.llm?.apiKey?.set) : true);
  }, []);

  useEffect(() => {
    void refreshLlmKeyState();
  }, [refreshLlmKeyState]);
  ...
  async function submitPrompt() {
    ...
    // The real guard. The textarea and the send button are both disabled below,
    // but neither is load-bearing: Enter is handled on keydown, and a disabled
    // control is a UI state, not a lock. Without this, a build starts, plans,
    // and fails several steps deep with "the agent reported a failure" — which
    // says nothing about the key, and leaves a broken app behind.
    if (llmKeySet === false) return;
    setRunning(true);
      ...
      <form onSubmit={handleSubmit} className="border-t border-border p-3">
        <div className="relative">
          <Textarea
            ...
            disabled={running || llmKeySet === false}
            aria-label="Describe the app to build"
            placeholder={
              llmKeySet === false
                ? 'Add your AI provider key to start building…'
                : hasExistingApp
                  ? 'Describe a change to this app…'
                  : 'Write your prompt…'
            }
            rows={3}
          ...
          {!running ? (
            <button
              type="submit"
              disabled={!prompt.trim() || llmKeySet === false}
        ...
        <p className="mt-1.5 px-0.5 text-[11px] text-muted-foreground">
          {llmKeySet === false ? (
            'Add your AI provider key to start building — the form is on the right.'
          ) : running ? (
  ...
  const previewPanelBody = (
    <div className="relative h-full min-h-0 bg-muted/30">
      ...
      <div className={cn('absolute inset-0', (showAdmin || showPublish || showSource) && 'hidden')}>
        ...
        ) : llmKeySet === false && !running ? (
          // No key means no build is possible, so the pane offers the one thing
          // that unblocks it rather than promising a preview that cannot come.
          <LlmKeyGate onSaved={() => void refreshLlmKeyState()} />
        ) : (
