...
export default function SecurityAdmin({ appId, mode }: { appId: string; mode: AdminMode }) {
  ...
  // Whether this instance has an outbound auth-email transport. Without one,
  // "Require email verification" would not merely be inert — it makes every
  // signup fail, since the backend withholds the session and then throws when
  // the verification mail can't be sent. So the switch is not offered.
            ...
            <Row
              ...
              desc={
                emailConfigured
                  ...
                  : 'Unavailable — this instance has no outbound email. Set RESEND_API_KEY and ' +
                    'EXEPAD_EMAIL_FROM (a domain you have verified with Resend), then restart. ' +
                    'Turning this on without them makes every new signup fail.'
              }
            >
              <Toggle
                checked={form.requireVerification}
                disabled={!emailConfigured && !form.requireVerification}
