...
export async function getSecurity(
  ...
  const { ok, data } = await api<{
    success?: boolean;
    data?: { security: AppSecurity | null; models: string[]; emailConfigured?: boolean };
    error?: string;
  ...
  return {
    ...
    // Default FALSE when the field is absent: an older runtime that doesn't
    // report this has no transport we can vouch for either, and the safe
    // reading is "don't offer the switch".
    emailConfigured: data.data?.emailConfigured === true,
    error: data.error,
  };
}
