...
const DAILY_STEPS = 8432;
const STEP_GOAL = 10000;
const STREAK_DAYS = 12;

function FitnessSidebar({ className }: { className?: string }) {
  const currentUser = useCurrentUser();
  const signout = useHandler("auth_signout", { autoFetch: false });
  const path = typeof window !== "undefined" ? window.location.pathname : "/";
  const currentPath = path.replace(/^\/[^/]+\/[^/]+\/[^/]+\/[^/]+/, "") || "/";

  const stepsPercent = Math.round((DAILY_STEPS / STEP_GOAL) * 100);
        ...
        <div className="px-3 mb-8">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-10 rounded-xl bg-primary flex items-center justify-center shadow-lg shadow-primary/20">
              <Icons.HeartPulse className="h-5 w-5 text-white" />
            </div>
            <div>
          ...
          {/* Streak Badge */}
          <div className="streak-badge rounded-lg px-3 py-2 flex items-center gap-2">
            <span className="text-lg">🔥</span>
            <div>
              <p className="text-white font-bold text-sm leading-none">
                {STREAK_DAYS} Day Streak
              </p>
              <p className="text-white/70 text-[10px] mt-0.5">Keep it up!</p>
            </div>
          </div>
        </div>
          ...
          <div className="px-3 py-3 bg-accent/50 rounded-lg mb-3">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <Icons.Footprints className="h-4 w-4 text-primary" />
            ...
            <DropdownMenuTrigger asChild>
              <button className="flex items-center gap-2 w-full px-2 py-1.5 rounded-md hover:bg-accent transition-colors">
                ...
                <div className="flex flex-col text-left text-xs leading-tight">
                  ...
                  <span className="text-muted-foreground truncate">{currentUser?.email || "user@fitpulse.io"}</span>
                </div>
              </button>
            </DropdownMenuTrigger>
            <DropdownMenuContent side="top" align="start" className="w-56">
              <DropdownMenuLabel>
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground">{currentUser?.email || "user@fitpulse.io"}</span>
                </div>
              </DropdownMenuLabel>
              <DropdownMenuSeparator />
