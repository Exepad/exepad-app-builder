...
# Synonyms a model reaches for instead of the four literal values.
#
# Observed live: qwen3.7-flash classified "create a landing page for a steel
# factory" as ``marketing``. Pydantic rejected the literal, ADK raised, and the
# build died at the FIRST phase — over a word, not a misjudgement. The field's
# own description says "'website' for content/marketing sites", so the model had
# the classification right and only used the wrong token for it.
#
# Mapping to the NEAREST VALID MEMBER, not to ``custom``: ``custom`` is not an
# inert catch-all. It gates the Three.js FPS recipe inlining
# (creation_workflow._should_inline_fps_recipe) and skips the website-specific
# backend suppression in backend_builder, so parking a marketing site there
# would quietly ship a worse app than the crash it replaced. ``custom`` remains
# the fallback only for values that genuinely match nothing.
_APP_TYPE_SYNONYMS: dict[str, str] = {
    ...
    "webpage": "website",
    "homepage": "website",
    # standalone form / survey apps
    "survey": "form",
    "quiz": "form",
    "questionnaire": "form",
    "poll": "form",
    # record-management dashboards
    "dashboard": "dataapp",
    "admin": "dataapp",
    "crm": "dataapp",
    "analytics": "dataapp",
...
_VALID_APP_TYPES = {"website", "form", "dataapp", "custom"}
...
class PreCreatorOutput(BaseModel):
    ...
    @field_validator("app_secondary_type", mode="before")
    @classmethod
    def _normalise_app_type(cls, value: object) -> object:
        """Map a near-miss classification onto the nearest valid member.

        Runs BEFORE the ``Literal`` check, inside ``model_validate_json`` — which
        is exactly what ADK calls — so no ADK patching is needed.

        Deliberately NOT silent: the warning carries the original value, because
        a coercion nobody can see is how a small model failure turns into a
        mysteriously mediocre app.
        """
        if not isinstance(value, str):
            return value
        cleaned = value.strip().lower().replace(" ", "").replace("_", "")
        if cleaned in _VALID_APP_TYPES:
            return cleaned
        mapped = _APP_TYPE_SYNONYMS.get(cleaned) or _APP_TYPE_SYNONYMS.get(
            cleaned.replace("-", "")
        )
        if mapped:
            logger.warning(
                "precreator_app_type_normalised",
                original=value,
                resolved=mapped,
                reason="synonym of a valid app type",
            )
            return mapped
        # Nothing matched — 'custom' IS the right bucket for a genuinely unknown
        # category, and shipping a custom-typed app beats failing the build.
        logger.warning(
            "precreator_app_type_unrecognised",
            original=value,
            resolved="custom",
            reason="no synonym match; falling back to the catch-all",
        )
        return "custom"
