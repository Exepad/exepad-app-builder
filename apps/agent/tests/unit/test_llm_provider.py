...
# ── EXEPAD_LLM_API_KEY on the native-Gemini path ─────────────────────────────
# EXEPAD_LLM_API_KEY is the LiteLLM path's variable — create_model reads it only
# AFTER the native-Gemini early return, and google-genai's Client is built with
# no api_key, falling back to GEMINI_API_KEY/GOOGLE_API_KEY. So on the DEFAULT
# provider the operator's key was silently ignored and every build failed to
# authenticate. Both first-run routes hit it: .env, and the Settings UI (which
# stores any provider's key as EXEPAD_LLM_API_KEY). config bridges the two.


@pytest.fixture(autouse=True)
def _reset_gemini_mirror(monkeypatch):
    """Clear the Google-side vars and the module's mirror bookkeeping."""
    import config as _config

    for var in ("GEMINI_API_KEY", "GOOGLE_API_KEY"):
        monkeypatch.delenv(var, raising=False)
    monkeypatch.setattr(_config, "_MIRRORED_GEMINI_KEY", None, raising=False)


def test_native_gemini_mirrors_exepad_key_into_gemini_api_key(monkeypatch):
    monkeypatch.setenv("EXEPAD_LLM_PROVIDER", "gemini")
    monkeypatch.setenv("EXEPAD_LLM_API_KEY", "AIza-from-dotenv")

    assert get_agent_model(AgentName.CREATOR) is not None
    assert os.environ["GEMINI_API_KEY"] == "AIza-from-dotenv"


def test_explicit_google_side_key_is_never_overwritten(monkeypatch):
    """An operator who set GEMINI_API_KEY themselves stays authoritative."""
    monkeypatch.setenv("EXEPAD_LLM_PROVIDER", "gemini")
    monkeypatch.setenv("EXEPAD_LLM_API_KEY", "AIza-from-settings-ui")
    monkeypatch.setenv("GEMINI_API_KEY", "AIza-operator-set")

    get_agent_model(AgentName.CREATOR)
    assert os.environ["GEMINI_API_KEY"] == "AIza-operator-set"


def test_explicit_google_api_key_also_blocks_the_mirror(monkeypatch):
    monkeypatch.setenv("EXEPAD_LLM_PROVIDER", "gemini")
    monkeypatch.setenv("EXEPAD_LLM_API_KEY", "AIza-from-settings-ui")
    monkeypatch.setenv("GOOGLE_API_KEY", "AIza-operator-set")

    get_agent_model(AgentName.CREATOR)
    assert os.environ.get("GEMINI_API_KEY", "") == ""


def test_non_gemini_provider_does_not_mirror(monkeypatch):
    """openrouter etc. pass the key to LiteLLM; nothing Google-side is touched."""
    monkeypatch.setenv("EXEPAD_LLM_PROVIDER", "openrouter")
    monkeypatch.setenv("EXEPAD_LLM_API_KEY", "sk-or-x")
    monkeypatch.setenv("EXEPAD_LLM_MODEL_DEFAULT", "deepseek/deepseek-v4-flash")
...
def test_settings_ui_path_mirrors_and_tracks_a_key_change(monkeypatch):
    """The Settings UI route, including rotating the key.

    A first-write-only mirror would leave the stale key in place on rotation, so
    ...
    _config.apply_runtime_settings({"llm": {"provider": "gemini", "api_key": "AIza-first"}})
    assert os.environ["GEMINI_API_KEY"] == "AIza-first"

    _config.apply_runtime_settings({"llm": {"provider": "gemini", "api_key": "AIza-rotated"}})
    assert os.environ["GEMINI_API_KEY"] == "AIza-rotated"
