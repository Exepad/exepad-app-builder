...
# The complete M3 @theme palette every valid theme.css must carry. The deploy
# step refuses to ship an app without all thirty tokens, so a constant named
# VALID_THEME_CSS that lacked them was not actually valid — it described a file
# that passes validation and then kills the build, which is the exact defect
# style.m3.palette_incomplete now catches at write time. Pure monochrome keeps
# every contrast pair at 21:1 so these fixtures still test structure only.
_M3_PALETTE = """\
@theme {
...
# The 24 tokens beyond the six a caller may want to set itself, for fixtures
# that pin their own primary/secondary/error pairs.
_M3_PALETTE_REMAINDER = """\
  --color-primary-container: #ffffff;
  --color-on-primary-container: #000000;
  --color-secondary-container: #ffffff;
  --color-on-secondary-container: #000000;
