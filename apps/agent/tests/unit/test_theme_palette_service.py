...
def test_incomplete_theme_degrades_to_seed_fill_instead_of_killing_the_build():
    """An incomplete theme.css used to RAISE, and that raise was terminal.

    A theme.css that was ABSENT fell back to a seed palette; one that was
    present but incomplete aborted the whole workflow — so a partly-bad theme
    was punished harder than no theme at all, and the user lost minutes of agent
    work over colour tokens. Both now degrade the same way.
    """
    snapshot = resolve_theme_palette_snapshot("@theme { --color-primary: #0f766e; }")

    assert snapshot.source == "theme_css_seed_filled"
    # The LLM's own value survives — the seed only fills the gaps, so a
    # partially-good palette keeps the identity the model chose.
    assert snapshot.palette["primary"] == "#0f766e"
    assert snapshot.palette["on-primary"]
    assert snapshot.palette["inverse-primary"]


def test_incomplete_theme_still_raises_when_seed_fallback_is_refused():
    """The strict path is preserved for callers that opt out of the fallback."""
    with pytest.raises(ThemePaletteResolutionError) as exc:
        resolve_theme_palette_snapshot(
            "@theme { --color-primary: #0f766e; }", fallback_to_seed=False
        )
...
def test_seed_fills_background_when_surface_alias_is_also_missing():
    """With BOTH background and its surface alias gone, no derivation is
    possible — the seed fills them rather than failing the build."""
    ...
    snapshot = resolve_theme_palette_snapshot(theme_no_bg_no_surface, fallback_to_seed=True)

    assert snapshot.source == "theme_css_seed_filled"
    assert snapshot.palette["background"]
    assert snapshot.palette["surface"]
    # Untouched tokens keep the theme's own values.
    assert snapshot.palette["primary"] == "#0f766e"


def test_incomplete_theme_raises_when_seed_fallback_refused_and_alias_impossible():
    theme_no_bg_no_surface = (
        VALID_THEME_CSS.replace("--color-background: #fffbeb;\n", "")
        .replace("--color-on-background: #1c1b1f;\n", "")
        .replace("--color-surface: #fffbeb;\n", "")
    ...
    with pytest.raises(ThemePaletteResolutionError) as exc:
        resolve_theme_palette_snapshot(theme_no_bg_no_surface, fallback_to_seed=False)
