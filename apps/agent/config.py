...
# Agents that carry BOTH an ``output_schema`` and ``tools`` — the exact set
# _patch_adk_output_schema_for_litellm routes through the SetModelResponseTool
# emission gate. Only these can safely have ``response_format`` dropped: the gate
# is their constraint, so the parameter is redundant, and it is also the one that
# fails (see the drop in _build_model_for_name).
#
# Everything else — PreCreator, AppHelpDesk, ChatResponseWriter — is tool-LESS,
# so response_format is the ONLY thing holding the output to shape. Dropping it
# for those was measured: the model returned its JSON inside a ```json fence and
# ``model_validate_json`` rejected it on both attempts.
_TOOL_EMISSION_GATE_AGENTS = frozenset(
    {
...
def _uses_tool_emission_gate(agent_name: "AgentName | str") -> bool:
    """True when this agent has tools alongside its output_schema."""
    name = agent_name.value if isinstance(agent_name, AgentName) else str(agent_name)
    return name in _TOOL_EMISSION_GATE_AGENTS
        ...
        litellm.register_model(
            {
                litellm_model_id: {
                    ...
                    # Stays True: tool-LESS agents (PreCreator, AppHelpDesk,
                    # ChatResponseWriter) rely on response_format as their only
                    # output constraint, and litellm drops the param entirely
                    # when this is False. The agents where response_format is
                    # both redundant and fatal are handled per-agent instead —
                    # see _TOOL_EMISSION_GATE_AGENTS.
...
def _build_model_for_name(model_name: str, *, drop_response_format: bool = False) -> BaseLlm:
    ...
    # Drop ``response_format`` ONLY for agents that also carry tools, and only on
    # providers using the SetModelResponseTool emission gate.
    #
    # Those requests carried BOTH mechanisms, and the redundant one is a hard
    # failure: OpenAI strict mode forbids free-form objects, CreatorOutput has
    # two (SecurityPlan.page_access / .role_hierarchy), and OpenRouter applies
    # that strict format to any model advertising support. Measured as a
    # deterministic 400 on openai/gpt-4o-mini AND anthropic/claude-sonnet-4.5:
    #
    #   Invalid schema for response_format 'CreatorOutput': ...
    #   Extra required key 'page_access' supplied.
    #
    # The narrow scope is load-bearing. Dropping it for EVERY agent was measured
    # and is worse: a tool-LESS agent (PreCreator) has no emission gate, so
    # response_format is its only constraint, and without it the model wrapped
    # its JSON in a ```json fence that model_validate_json cannot parse. Only
    # tool-carrying agents have the gate to fall back on.
    #
    # Done here rather than via the capability registry because
    # _register_litellm_model early-returns for ids litellm already knows — so
    # metadata alone still sent the param for mapped models.
    # ``drop_params`` is what makes litellm honour ``additional_drop_params``.
    if drop_response_format and not litellm_model_id.startswith(
        _NATIVE_STRUCTURED_LITELLM_PREFIXES
    ):
        kwargs["drop_params"] = True
        kwargs["additional_drop_params"] = ["response_format"]
...
def get_agent_model(agent_name: AgentName | str) -> BaseLlm:
    ...
    Raises:
        KeyError: If agent_name is not found in AGENT_MODELS
    """
    return _build_model_for_name(
        get_agent_model_name(agent_name),
        drop_response_format=_uses_tool_emission_gate(agent_name),
    )
...
def rebind_runtime_models() -> int:
    ...
    for agent in _collect_llm_agents(LlmAgent, BaseAgent):
        ...
        if not model_name:
            continue
        try:
            # Same drop decision as get_agent_model — the rebinder replaces the
            # model object wholesale, so without this a rebound Creator would
            # silently regain the response_format that 400s.
            agent.model = _build_model_for_name(
                model_name,
                drop_response_format=_uses_tool_emission_gate(getattr(agent, "name", "")),
            )
            count += 1
