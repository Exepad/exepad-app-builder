...
# identically on 22. Keep the version in step with package.json's
# packageManager field by hand: nothing reads that field without corepack.
#
# NOTE for whoever next tries node >= 25: corepack is only the FIRST blocker.
# better-sqlite3 has no prebuilt binary for that ABI yet, so pnpm install falls
# back to compiling it with node-gyp, which needs a Python toolchain this slim
# builder image does not carry:
#   gyp ERR! stack Error: Could not find any Python installation to use
# Bumping Node therefore also means adding python3 + build-essential to this
# stage (and paying the compile), or waiting for upstream prebuilds.
...
# =============================================================================
# NOT python:3.14 yet. asyncpg and tree-sitter publish no wheels for 3.14, so
# pip falls back to building them from source and this slim image has no gcc:
#   error: command 'gcc' failed: No such file or directory
#   Failed to build asyncpg tree-sitter
# Same shape as the node>=25 blocker above: the runtime is newer than the
# native-dependency ecosystem. Bumping means adding a compiler toolchain to
# this stage, or waiting for upstream wheels.
