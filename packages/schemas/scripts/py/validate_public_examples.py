#!/usr/bin/env python3
"""
Validate all app configs under apps/runtime/public/example/.

Usage:
    python validate_public_examples.py

Skips non-config directories (data, repo, debug, backend/handlers, assets, patterns).
...
PUBLIC_EXAMPLES_DIR = os.path.normpath(
    os.path.join(_current_dir, "..", "..", "..", "..", "apps", "runtime", "client", "public", "example")
)
REPORT_PATH = os.path.join(_current_dir, "public_examples_validation_report.txt")
...
def main():
    print(f"Validating public example configs...")
    print(f"Directory: {PUBLIC_EXAMPLES_DIR}\n")

    if not os.path.isdir(PUBLIC_EXAMPLES_DIR):
        print(f"{RED}Error: directory not found: {PUBLIC_EXAMPLES_DIR}{RESET}")
        sys.exit(1)

    files = walk_json_files(PUBLIC_EXAMPLES_DIR)
    total_errors = 0
    total_warnings = 0
    ...
    if report_lines:
        ...
        header = (
            ...
            f"Directory: {PUBLIC_EXAMPLES_DIR}\n"
    ...
    if total_errors > 0:
        ...
        print(f"\n{GREEN}{BOLD}All public examples pass validation.{RESET}")
