...
export async function deploy(ctx: Context, flags: Flags, mode: 'up' | 'update'): Promise<number> {
  ...
  // Tokenless first-run setup — the same default install.sh and install.ps1 set.
  // The setup token proves "I can read this host's logs", which for the ordinary
  // case means making someone dig through `docker logs` to prove they own the
  // machine they are sitting at; the studio hides the field when it is not
  // required. The trade: until the setup form is completed, anyone who can reach
  // the published port can complete it instead of you.
  //
  ...
  // `openSetup` means "open setup is IN EFFECT", not "this run added it" — a
  // re-run over an .env that already carries the flag must reach the same
  // conclusion, or it skips the "create your account" hint and shells out
  // looking for a token that was never minted.
  const explicitOpenSetup = /^EXEPAD_ALLOW_OPEN_SETUP=(.*)$/m.exec(existingEnv);
  const openSetup = explicitOpenSetup
    ? /^(1|true|yes|on)$/i.test(explicitOpenSetup[1].trim())
    : true;
  if (!explicitOpenSetup) updates.EXEPAD_ALLOW_OPEN_SETUP = '1';

  // No interactive key prompt, deliberately — the same rule install.sh and
  // install.ps1 follow. Configuration, the AI provider key above all, belongs in
  // the studio's Settings UI: one place to set it, validate it and rotate it.
  // --llm-key / EXEPAD_LLM_API_KEY still work, for unattended installs that have
  // no UI to go to. Having no key yet is the NORMAL state after an interactive
  // install, so it is reported at the end as a next step, not here as a warning.
  const needsKey =
    mode === 'up' && !hasEnvKey(existingEnv, updates, 'EXEPAD_LLM_API_KEY');
  ...
  // Open setup mints no token, so skip the log read rather than shell out for a
  // banner that will never be there.
  if (!openSetup) printSetupToken(ctx, dir);
  ctx.out('');
  if (openSetup && mode === 'up') {
    ctx.out('  Next: open it and create your operator account — do this now, while');
    ctx.out('  nobody else can reach the port first.');
  }
  if (needsKey) {
    ctx.out('  Then add your AI provider key in the studio — Settings → AI provider.');
    ctx.out('  Builds need it; everything else works without it.');
  }
  return 0;
}

/**
 * Show the first-run setup token, which the setup screen demands before an
 * operator account can be created.
 *
 * The container prints it to STDERR once at boot (docker/entrypoint.sh), but
 * every install path starts the container DETACHED — so that banner goes
 * straight to the logs and the user never sees it. They then hit a screen
 * asking for a token nothing has shown them, and the only way out was to know
 * `docker logs … | grep`, which is not something a Windows user can run at all.
 *
 * So read it back and print it. Best-effort by design: a failure here must not
 * fail an otherwise successful `up`, and there is legitimately nothing to show
 * when the operator seeded EXEPAD_ADMIN_EMAIL/PASSWORD (no token is minted) or
 * on a re-run of an instance whose account already exists.
 */
function printSetupToken(ctx: Context, dir: string): void {
  try {
    const res = ctx.exec(verbEngine(ctx, dir), ['logs', CONTAINER_NAME], { cwd: dir });
    // The banner is on stderr; merge both so we do not depend on that detail.
    const logs = `${res.stdout || ''}\n${res.stderr || ''}`;
    // 64 hex chars on its own line, the shape entrypoint.sh emits.
    const m = logs.match(/FIRST-RUN SETUP TOKEN[\s\S]{0,200}?([a-f0-9]{64})/i);
    if (!m) return;
    ctx.out('');
    ctx.out(`  Setup token: ${m[1]}`);
    ctx.out('  Enter it on the first screen to create your operator account.');
  } catch {
    /* never let a cosmetic lookup fail the install */
  }
}
